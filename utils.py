...
def get_action_v2(id_action, point, obs):
        ...
        if (obs.observation['feature_minimap'][_PLAYER_RELATIVE] == _PLAYER_HOSTILE).nonzero():         # 攻击已知敌人
            ...
            if len(obs.observation['multi_select']):
                if obs.observation['multi_select'][0][0] != _TERRAN_SCV and _ATTACK_MINIMAP in obs.observation["available_actions"]:
                    ...
                    func = actions.FunctionCall(_ATTACK_MINIMAP, [_NOT_QUEUED, target])
    #
    # elif not army_rallied:
    #     if not army_selected:
    #         if _SELECT_ARMY in obs.observation["available_actions"]:
    #             army_selected = True
    ...
    #
    #         if base_top_left:
    #             func = actions.FunctionCall(_ATTACK_MINIMAP, [_NOT_QUEUED, [39, 45]])
    #
    #         func = actions.FunctionCall(_ATTACK_MINIMAP, [_NOT_QUEUED, [21, 24]])
    #
    # elif smart_action == ACTION_ATTACK:
    #     if _ATTACK_MINIMAP in obs.observation["available_actions"]:
    #         return actions.FunctionCall(_ATTACK_MINIMAP, [_NOT_QUEUED, transformLocation(obs, int(x), int(y))])
...
class action_v2():
    ...
    def select_scv(self):
        # if smart_action == ACTION_SELECT_SCV:
        unit_type = self.obs.observation['feature_screen'][_UNIT_TYPE]
        unit_y, unit_x = (unit_type == _TERRAN_SCV).nonzero()
    ...
    def train_scv(self):
        # elif smart_action == ACTION_TRAIN_SCV:
        worker_cnt = 0
        if _TRAIN_SCV in self.obs.observation['available_actions'] and worker_cnt < 16:
            func = actions.FunctionCall(_TRAIN_SCV, [_QUEUED])
            return func
    ...
    def build_supply_depot(self, point):
        # elif smart_action == ACTION_BUILD_SUPPLY_DEPOT:
        if _BUILD_SUPPLY_DEPOT in obs.observation['available_actions']:
            unit_type = self.obs.observation['feature_screen'][_UNIT_TYPE]
            unit_y, unit_x = (unit_type == _TERRAN_COMMANDCENTER).nonzero()
    ...
    def attack(self, point):
        # elif smart_action == ACTION_ATTACK:
        enemy_y, enemy_x = (self.self.obs.observation['feature_minimap'][_PLAYER_RELATIVE] == _PLAYER_HOSTILE).nonzero()

        if (self.obs.observation['feature_minimap'][_PLAYER_RELATIVE] == _PLAYER_HOSTILE).nonzero():
            for i in range(0, len(enemy_y)):
                if len(self.obs.observation['multi_select']):
                    if self.obs.observation['multi_select'][0][0] != _TERRAN_SCV and _ATTACK_MINIMAP in self.obs.observation["available_actions"]:
                        # if _ATTACK_MINIMAP in obs.observation["available_actions"]:
                        if enemy_y.any():
                            target = [int(enemy_x.mean()), int(enemy_y.mean())]
                            func = actions.FunctionCall(_ATTACK_MINIMAP, [_NOT_QUEUED, target])
    ...
    def build_engbay(self, point):
        # elif smart_action == ACTION_BUILD_ENGBAY:
        if _BUILD_ENG_BAY in self.obs.observation['available_actions']:
            unit_type = self.obs.observation['feature_screen'][_UNIT_TYPE]
            unit_y, unit_x = (unit_type == _TERRAN_COMMANDCENTER).nonzero()
    ...
    def build_missle_turrent(self, point):
        # elif smart_action == ACTION_BUILD_MISSLE_TURRENT:
        if _BUILD_MISSLE_TURRENT in self.obs.observation['available_actions']:
            unit_type = self.obs.observation['feature_screen'][_UNIT_TYPE]
            unit_y, unit_x = (unit_type == _TERRAN_SUPPLY_DEPOT).nonzero()
    ...
    def correct_available_actions(self, func, smart_action):
        try:
            return func, smart_action
